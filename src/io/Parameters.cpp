...
namespace RefactorProcessing{
    Parameters::Parameters() : program("rnaAligner") {
        ...
        program.add_argument("--threads")
                .help("number of working threads to use , default 1")
                .default_value(1)
                .scan<'i',int>();
        ...
        program.add_argument("--genomeGenerateFileStoreDir")
                ...
                .default_value(std::string("./"));
        program.add_argument("--outErrorFile")
                .help("the file to save error messages, default out.error.log")
                .default_value(std::string("out.error.log"));
        program.add_argument("--outWarningFile")
                .help("the file to save warning messages, default out.warning.log")
                .default_value(std::string("out.warning.log"));
    ...
    int Parameters::process(int argc, char* argv[]){
            ...
            workingThreads = program.get<int>("--threads");
            ...
            outErrorFile = outPutDir+program.get<std::string>("--outErrorFile");
            outWarningFile = outPutDir+program.get<std::string>("--outWarningFile");
            outProgressFile = outPutDir+program.get<std::string>("--outProgressFile");
            sjdbOverhang = program.get<int>("--sjdbOverhang");
            sjdbLength = sjdbOverhang * 2 + SJDB_PADDING_LENGTH;
            ...
            for (int i = 0; i < 4; ++i) MAX_MISMATCH_FOR_SJ[i] = maxMismatchForSJVec[i];
